...
func NewConn() *Conn {
	...
	conn.c = (*C.conn)(C.calloc(1, C.sizeof_conn))
...
func (conn *Conn) GetError(code C.int) error {
	...
	msg := C.GoStringN(conn.c.errstr.buf_addr, C.int(min(conn.c.errstr.len_used, conn.c.errstr.len_alloc)))
...
func (conn *Conn) Node(varname string, subscripts ...string) (n *Node) {
	...
	runtime.AddCleanup(n, func(cn *C.node) {
		C.free(unsafe.Pointer(cn))
	}, n.n)

	n.conn = conn // point to the Go conn
	cn := n.n
	cn.conn = (*C.conn)(unsafe.Pointer(conn.c)) // point to the C version of the conn
	cn.len = C.int(len(subscripts) + 1)
	cn.mutable = 0 // i.e. false

	dataptr := unsafe.Add(unsafe.Pointer(&cn.buffers[0]), C.sizeof_ydb_buffer_t*(len(subscripts)+1))
	C.memcpy(dataptr, unsafe.Pointer(&joiner.Bytes()[0]), C.size_t(joiner.Len()))

	// Now fill in ydb_buffer_t pointers
	s := varname
	buf := (*C.ydb_buffer_t)(unsafe.Pointer(&cn.buffers[0]))
	...
	for i, s := range subscripts {
		buf := (*C.ydb_buffer_t)(unsafe.Add(unsafe.Pointer(&cn.buffers[0]), C.sizeof_ydb_buffer_t*(i+1)))
		buf.buf_addr = (*C.char)(dataptr)
		buf.len_used, buf.len_alloc = C.uint(len(s)), C.uint(len(s))
		dataptr = unsafe.Add(dataptr, len(s))
	}
...
func (n *Node) String() string {
	...
	cn := n.n // access C.node from Go node
	for i := range cn.len {
		buf := (*C.ydb_buffer_t)(unsafe.Add(unsafe.Pointer(&cn.buffers[0]), C.sizeof_ydb_buffer_t*i))
...
func (n *Node) Set(val string) error {
	...
	ret := C.ydb_set_st(conn.tptoken, &conn.errstr, &cn.buffers[0], cn.len-1, (*C.ydb_buffer_t)(unsafe.Add(unsafe.Pointer(&cn.buffers[0]), C.sizeof_ydb_buffer_t)), &conn.value)
...
func (n *Node) Get(deflt ...string) (string, error) {
	cn := n.n // access C.node from Go node
	conn := cn.conn
	err := C.ydb_get_st(conn.tptoken, &conn.errstr, &cn.buffers[0], cn.len-1, (*C.ydb_buffer_t)(unsafe.Add(unsafe.Pointer(&cn.buffers[0]), C.sizeof_ydb_buffer_t)), &conn.value)
